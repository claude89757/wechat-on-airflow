...
def summary_chat_history(**context):
    ...
    if not contact_name:
        raise ValueError("缺少必填参数: contact_name")
    if not wx_user_id:
        raise ValueError("缺少必填参数: wx_user_id")

    # 获取聊天记录
    chat_history = get_wx_chat_history(contact_name=contact_name, wx_user_id=wx_user_id, limit=100)
    print(f"获取到 {len(chat_history)} 条聊天记录")
    
    if not chat_history:
        print("未获取到聊天记录，请检查contact_name和wx_user_id是否正确")
    ...
    print("response_data回复",response_data,"---------")
