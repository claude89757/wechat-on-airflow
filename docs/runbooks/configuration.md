...
five Fansibote chain courts (深云, 蛇口, 新安, 正中, 安托山) query the same
...
FFTENNIS uses the mini-program's documented public request headers and performs
a TLS-verified direct request first, falling back to the shared HTTPS proxy pool
only when direct access fails. It publishes only 1–6号双打场 and drops 7–8号
非标单打场 plus 9–10号发球机场. The chain-court adapter keeps only standard
...
`FSB_PROXY_CACHE`, `FFT_PROXY_CACHE`, `PPBA_PROXY_CACHE`, notification baseline,
and venue dedupe caches are created on the first successful run.
