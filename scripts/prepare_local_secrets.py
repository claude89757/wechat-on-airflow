...
def secret_contract() -> tuple[Path, tuple[str, ...]]:
...
def main() -> None:
    ...
    default_directory, filenames = secret_contract()
    directory = (args.directory or default_directory).resolve()
    directory.mkdir(mode=0o700, parents=True, exist_ok=True)
    directory.chmod(0o700)
    created = sum(create_secret(directory / filename) for filename in filenames)
