...
latest_observation.json包含本轮恢复原生URL启动顺序之后的新观察，优先以该文件中的实测状态判断进展，不能继续把旧失败当成当前状态。
...
def service_roots() -> tuple[list[Path], list[str]]:
    ...
    for prop in ("WorkingDirectory", "ExecStart"):
        ...
        if prop == "ExecStart":
            for entry in re.findall(r"(/[A-Za-z0-9_./-]+/apps/cli/lib/bin\.js)", result):
                roots.append(Path(entry).parent.parent)
    ...
    return roots, list(dict.fromkeys(node_paths))
