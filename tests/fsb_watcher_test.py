...
class FsbWatcherTest(unittest.TestCase):
    ...
    def test_parse_availability_normalizes_end_times_and_merges_slots(self) -> None:
        result = fsb_watcher.parse_availability(
            {
                ...
                "result": {
                    "slots": [
                        {
                            "classRoomName": "1号风雨场",
                            "beginDatetime": "2026-08-24 18:00:00",
                            "endDatetime": "2026-08-24 18:59:00",
                            "apptInfo": {"canApptOrNot": True},
                        },
                        {
                            "classRoomName": "1号风雨场",
                            "beginDatetime": "2026-08-24 19:00:00",
                            "endDatetime": "2026-08-24 19:59:00",
                            "apptInfo": {"canApptOrNot": True},
                        },
                        {
                            "classRoomName": "1号风雨场",
                            "beginDatetime": "2026-08-24 20:00:00",
                            "endDatetime": "2026-08-24 20:59:00",
                            "apptInfo": {"canApptOrNot": False},
                        },
                        {
                            "classRoomName": "1号风雨场",
                            "beginDatetime": "2026-08-24 23:00:00",
                            "endDatetime": "2026-08-24 23:59:00",
                            "apptInfo": {"canApptOrNot": True},
                        },
                    ]
                },
    ...
    def test_filter_slots_uses_weekday_and_weekend_windows(self) -> None:
        ...
        weekend_result = fsb_watcher.filter_court_data_for_notification(
            "2026-08-23",
            {"2号风雨场": [["15:00", "16:00"], ["16:00", "17:00"]]},
        )
    ...
    def test_build_new_notifications_skips_already_sent_messages(self) -> None:
        messages = fsb_watcher.build_new_notifications(
            [
                {
                    "date": "08-24",
                    "court_name": "泛思博特福中福1号风雨场",
            ...
            ["【泛思博特福中福1号风雨场】星期一(08-24)空场: 18:00-19:00"],
            current_year=2026,
        )

        self.assertEqual(
            messages,
            ["【泛思博特福中福1号风雨场】星期一(08-24)空场: 19:00-20:00"],
        )

    def test_empty_proxy_list_does_not_call_api_directly(self) -> None:
        original_post = fsb_watcher.requests.post

        def fail_if_called(*args: object, **kwargs: object) -> None:
            raise AssertionError("FSB API should not be called directly without a proxy")

        fsb_watcher.requests.post = fail_if_called
        try:
            with self.assertRaisesRegex(Exception, "all proxies failed"):
                fsb_watcher.get_tennis_court_availability("2026-08-24", [])
        finally:
            fsb_watcher.requests.post = original_post

    def test_check_tennis_courts_publishes_webapp_before_wechat(self) -> None:
        expected_msg = "【泛思博特福中福1号风雨场】星期日(08-23)空场: 16:00-18:00"
        original_load_proxy = fsb_watcher.load_proxy_list
        original_get_availability = fsb_watcher.get_tennis_court_availability
        original_publish = fsb_watcher.publish_venue_observation
        original_enqueue = fsb_watcher.enqueue_wechat_message
        original_datetime = fsb_watcher.datetime
        ...
        fsb_watcher.load_proxy_list = lambda: []
        fsb_watcher.get_tennis_court_availability = fake_get_availability
        fsb_watcher.publish_venue_observation = fake_publish
        fsb_watcher.enqueue_wechat_message = fallback_wechat
        fsb_watcher.datetime = FixedDatetimeModule
        try:
            fsb_watcher.run_check_tennis_courts()
            self.assertIn(expected_msg, FakeVariable.values.get(fsb_watcher.CACHE_KEY, []))
            self.assertEqual(events, ["webapp", "wechat"])
            self.assertEqual(observations[0][0][0:2], ("fsb", "泛思博特福中福"))
        finally:
            fsb_watcher.load_proxy_list = original_load_proxy
            fsb_watcher.get_tennis_court_availability = original_get_availability
            fsb_watcher.publish_venue_observation = original_publish
            fsb_watcher.enqueue_wechat_message = original_enqueue
            fsb_watcher.datetime = original_datetime
