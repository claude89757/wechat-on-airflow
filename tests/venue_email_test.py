...
class VenueEmailTest(unittest.TestCase):
    def setUp(self):
        self.variables = {
            venue_email.SZ_TENNIS_EMAIL_LIST_VAR: [
                "a@example.com",
                "b@example.com",
            ],
    ...
    def test_batch_sends_all_new_slots_in_one_email(self):
        ...
        with (
            ...
            result = venue_email.send_venue_email_batch("深圳湾网球场巡检", notifications)
    ...
    def test_failed_send_is_recorded_without_raising(self):
        ...
        with (
            ...
            result = venue_email.send_venue_email_batch("上越沙河网球场巡检", notifications)
    ...
    def test_missing_global_recipient_variable_uses_fallback(self):
        self.variables.pop(venue_email.SZ_TENNIS_EMAIL_LIST_VAR)
        notifications = [
            {
                "date": "07-15",
                "court_name": "TOPS1号场",
        ...
        with (
            ...
            result = venue_email.send_venue_email_batch("TOPS科技园网球场巡检", notifications)

        self.assertFalse(result["success"])
        self.assertIn(venue_email.SZ_TENNIS_EMAIL_LIST_VAR, result["error"])
        mock_send.assert_not_called()
        self.assertEqual(len(self.variables[venue_email.EMAIL_SEND_FALLBACK_OUTBOX_VAR]), 1)

    def test_all_active_venue_watchers_use_global_batch_helper(self):
        ...
        for filename in watcher_files:
            source = (watcher_dir / filename).read_text(encoding="utf-8")
            with self.subTest(filename=filename):
                self.assertIn("send_venue_email_batch", source)
                self.assertNotIn('Variable.get("SZW_EMAIL_LIST"', source)
                self.assertNotIn('Variable.get("JDWX_EMAIL_LIST"', source)
