  ...
  return (
    <BottomSheet open={open} onOpenChange={onOpenChange} title="加入 Zacks 网球群"
      ...
      <div className="wechat-group" data-testid="wechat-group">
        <button className="wechat-group-close" type="button" aria-label="关闭入群说明" onClick={() => onOpenChange(false)}><XIcon size={20} /></button>
        ...
        <div className="wechat-group-actions">
          <button type="button" onClick={() => void copyWechat()}>{feedback === "已复制微信号" ? <CheckIcon size={18} /> : <CopyIcon size={18} />}复制微信号</button>
          {!imageFailed ? <a href={WECHAT_QR} download="Zacks-wechat.png" target="_blank" rel="noopener noreferrer"><DownloadSimpleIcon size={18} />保存二维码</a> : null}
        </div>
