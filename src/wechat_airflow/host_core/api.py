...
from .service import increment_subscription_generations, ingest_observation, runtime_heartbeat
...
def _client_ip_hash(request: Request, settings: HostCoreSettings) -> str:
    return hashlib.sha256(
        f"{_client_ip(request)}:{settings.verification_pepper}".encode()
    ).hexdigest()


def _constant_time_header(value: str, expected: str) -> bool:
    return bool(value) and hmac.compare_digest(value.encode(), expected.encode())
...
def _internal_authorized(request: Request, settings: HostCoreSettings) -> bool:
    ...
    return authorization.startswith("Bearer ") and _constant_time_header(
        authorization[7:].strip(), settings.edge_token
    )
...
async def protect_origin(request: Request, call_next):  # type: ignore[no-untyped-def]
    ...
    settings = _settings()
    allowed = (
        _internal_authorized(request, settings)
        if request.url.path.startswith(f"{API_PREFIX}/internal/")
        else _edge_authorized(request, settings)
    )
    if not allowed:
...
def _tier(connection: Any, email: str) -> str:
...
def _is_admin(connection: Any, email: str) -> bool:
    return bool(
        connection.execute(
            text(
                """
        ...
        connection.execute(
            ...
            ),
            {
                ...
                "code_hash": hash_verification_code(
                    challenge_id, code, settings.verification_pepper
                ),
                "ip_hash": ip_hash,
...
def community_users(request: Request) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        activity = (
            "今天" if age < timedelta(days=1) else "本周" if age < timedelta(days=7) else "较早"
        )
        ...
        rows = (
            connection.execute(
                ...
                SELECT p.*,
                       ...
                       count(DISTINCT outbox.message_id) FILTER (
                           WHERE outbox.submitted_at >= :day_start
                       ) AS submitted_today,
                       count(DISTINCT outbox.message_id) FILTER (
                           WHERE outbox.status = 'delivered'
                             AND outbox.delivered_at >= :day_start
                       ) AS delivered_today,
                       count(DISTINCT outbox.message_id) FILTER (
                           WHERE outbox.status = 'failed'
                             AND outbox.failed_at >= :day_start
                       ) AS failed_today,
                       count(DISTINCT outbox.message_id) FILTER (
                           WHERE outbox.status = 'delivered'
        ...
        "users": [
            {
                ...
                "activeSubscriptions": int(row["active_subscriptions"] or 0),
                "submittedToday": int(row["submitted_today"] or 0),
                "deliveredToday": int(row["delivered_today"] or 0),
                "failedToday": int(row["failed_today"] or 0),
...
def update_invite(
    invite_id: str, request: Request, payload: dict[str, Any] = Body(...)
) -> dict[str, Any]:
    _require_admin(request)
    now = utc_now()
    with transaction() as connection:
