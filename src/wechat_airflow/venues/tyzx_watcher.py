...
def set_md5(timestamp: int, content: str | None = None) -> str:
    """
    生成MD5签名
...
class TennisCourtAPI:
    ...
    def _make_request(self, method, endpoint, data=None, **kwargs):
        ...
        headers.update({"timestamp": str(timestamp), "sign": sign})
        ...
        if unavailable_slots:
            for slot in unavailable_slots[:3]:  # 只显示前3个，避免日志太长
                ...
                if slot.get("locked", False):
                    ...
                    print(
                        f"      ⚠️ 发现逻辑错误: {court_name} {slot['time']} appointFlag=1但remainNum={slot['remainNum']}>0 却被标记为不可预约!"
                    )
                else:
...
def get_free_tennis_court_infos_for_tyzx(date: str, proxy_list: list) -> dict:
    ...
    print(
        f"📋 合并后代理总数: {len(all_proxies)} (缓存: {len(cached_proxies)}, 新增: {len(proxy_list) - len([p for p in proxy_list if p in cached_proxies])})"
    )
    ...
    for i, index in enumerate(all_indices):
        ...
        try:
            ...
            if "error" not in result and result.get("body"):
                ...
                print(
                    f"📦 API响应摘要: code={result.get('code')}, msg='{result.get('msg')}', 数据条数={len(result.get('body', []))}"
                )
                got_response = True
                ...
                print(
                    f"❌ 代理 {proxy} 响应无效: code={result.get('code', 'N/A')}, msg='{result.get('msg', 'N/A')}'"
                )
                # 如果是缓存的代理失败了，从缓存中移除
...
def run_check_tennis_courts():
    ...
    if up_for_send_data_list:
        ...
        for data in up_for_send_data_list:
            ...
            print(
                f"📅 处理 {court_name} 在 {date} (星期{weekday_str}) 的 {len(free_slot_list)} 个时段:"
            )
